...
# -----------------------------
# Main Function: generate_road_dataset
# -----------------------------
def generate_road_dataset(start_lat, start_lon, finish_lat, finish_lon, radius=2000):
    """
    Generate a road dataset with computed metrics from a given area defined by a start and finish point.

    Inputs:
      start_lat, start_lon, finish_lat, finish_lon: Coordinates for start and finish points.
      radius: Radius (in meters) around the start point for downloading road network and POIs.

    Returns:
      A GeoDataFrame with final road segments including:
      - OSMnx attributes (u, v, osmid, highway, lanes, maxspeed, name, oneway, reversed, junction, width)
      - length (computed if not present)
      - Venue counts: shop_count, tourism_count, amenity_count, public_transport_count
      - Traffic metric: traffic_score
      - Weather metrics: weather_main, weather_desc, temperature, feels_like, pressure, humidity, wind_speed, wind_deg
      - geometry
    """
    # Check finish point distance
...
if __name__ == '__main__':
    try:
        with open("config.json") as f:
            config = json.load(f)
        # Load configuration (already loaded above)
        ...
        # Optional: visualize final dataset (base visualization)
        ...
        # Write final dataset to GeoJSON file
